...
def run_training(config: Hyperparameters, model, optimizer, scheduler, tokenizer, train_loader: DataLoader, val_loader: DataLoader):
    ...
    for epoch in range(config.epochs):
        avg_train_loss = train_one_epoch(model, train_loader, optimizer, scheduler, epoch, config, tokenizer, val_loader)
        model.eval()
        wandb.log({
